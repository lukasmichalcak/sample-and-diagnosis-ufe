...
import { SampleAndDiagnosisTestListApi, SampleAndDiagnosisConditionsApi, TestListEntry, Configuration, Condition } from '../../api/sample-and-diagnosis-test';
...
export class MglmSampleAndDiagnosisTestEditor {
  ...
  async componentWillLoad() {
      this.getTestEntryAsync();
      this.getConditions();
  }

  private async getTestEntryAsync(): Promise<TestListEntry> {
    if(this.entryId === "@new") {
        this.isValid = false;
        this.entry = {
          id: "@new",
    ...
    try {
      ...
      const response = await testListApi.getTestListEntryRaw({sampleAndDiagnosisId: this.sampleAndDiagnosisId, entryId: this.entryId});

      if (response.raw.status < 299) {
          this.entry = await response.value();
          this.isValid = true;
      } else {
          this.errorMessage = `Cannot retrieve list of test patients: ${response.raw.statusText}`
      }
    } catch (err: any) {
  ...
  private async getConditions(): Promise<Condition[]> {
    try {
      ...
      const conditionsApi = new SampleAndDiagnosisConditionsApi(configuration);

      const response = await conditionsApi.getConditionsRaw({sampleAndDiagnosisId: this.sampleAndDiagnosisId})
      if (response.raw.status < 299) {
  ...
  private async updateEntry() {
    ...
    try {
      ...
      await testListApi.createTestListEntryRaw({sampleAndDiagnosisId: this.sampleAndDiagnosisId, testListEntry: this.entry}) :
      await testListApi.updateTestListEntryRaw({sampleAndDiagnosisId: this.sampleAndDiagnosisId, entryId: this.entryId, testListEntry: this.entry});
  ...
  private async deleteEntry() {
    try {
      ...
      const response = await testListApi.deleteTestListEntryRaw({sampleAndDiagnosisId: this.sampleAndDiagnosisId, entryId: this.entryId});
        if (response.raw.status < 299) {
